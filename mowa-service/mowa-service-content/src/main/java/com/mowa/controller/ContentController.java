...
import com.mowa.service.ContentService;
...
public class ContentController {
    ...
    public Result<List<Content>> findList(@RequestBody(required = false) @ApiParam(name = "Content对象",value = "传入JSON数据",required = false) Content content){
        ...
        return new Result<List<Content>>(true,StatusCodeEnum.SUCCESS.getCode(),"查询成功",list);
    }
    ...
    @ApiImplicitParam(paramType = "path", name = "id", value = "主键ID", required = true, dataType = "Integer")
    @PutMapping(value="/{id}")
    public Result update(@RequestBody @ApiParam(name = "Content对象",value = "传入JSON数据",required = false) Content content,@PathVariable Integer id){
        //设置主键值
        content.setId(id);
    ...
    @ApiImplicitParam(paramType = "path", name = "id", value = "主键ID", required = true, dataType = "Integer")
    @GetMapping("/{id}")
    public Result<Content> findById(@PathVariable Integer id){
        //调用ContentService实现根据主键查询Content
        Content content = contentService.findById(id);
        return new Result<Content>(true,StatusCodeEnum.SUCCESS.getCode(),"查询成功",content);
    }
