...
import com.mowa.service.ContentCategoryService;
...
public class ContentCategoryController {
    ...
    public Result<List<ContentCategory>> findList(@RequestBody(required = false) @ApiParam(name = "ContentCategory对象",value = "传入JSON数据",required = false) ContentCategory contentCategory){
        ...
        return new Result<List<ContentCategory>>(true,StatusCodeEnum.SUCCESS.getCode(),"查询成功",list);
    }
    ...
    @ApiImplicitParam(paramType = "path", name = "id", value = "主键ID", required = true, dataType = "Integer")
    @PutMapping(value="/{id}")
    public Result update(@RequestBody @ApiParam(name = "ContentCategory对象",value = "传入JSON数据",required = false) ContentCategory contentCategory,@PathVariable Integer id){
        //设置主键值
    ...
    @ApiImplicitParam(paramType = "path", name = "id", value = "主键ID", required = true, dataType = "Integer")
    @GetMapping("/{id}")
    public Result<ContentCategory> findById(@PathVariable Integer id){
        //调用ContentCategoryService实现根据主键查询ContentCategory
        ContentCategory contentCategory = contentCategoryService.findById(id);
        return new Result<ContentCategory>(true,StatusCodeEnum.SUCCESS.getCode(),"查询成功",contentCategory);
    }
